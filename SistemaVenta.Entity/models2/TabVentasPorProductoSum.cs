using System;
using System.Collections.Generic;

namespace SistemaVenta.Entity.models2
{
    public partial class TabVentasPorProductoSum
    {
        public int Idproducto { get; set; }
        public string Codigo { get; set; } = null!;
        public decimal? PromVtas { get; set; }
        public DateTime? UltVtaFec { get; set; }
        public decimal? TotVtas { get; set; }
        public int? Stock { get; set; }
        public int? UndVtaCalc { get; set; }
        public int UltCantIngreso1 { get; set; }
        public int CantVtas1 { get; set; }
        public int CantClien1 { get; set; }
        public int DiasDemoraImport1 { get; set; }
        public int DiasConStock1 { get; set; }
        public int MesesConStock1 { get; set; }
        public int DiasDemoraImport2 { get; set; }
        public int DiasConStock2 { get; set; }
        public int MesesConStock2 { get; set; }
        public int CantPorLlegar { get; set; }
        public int DiasDemoraImportPorLlegar { get; set; }
        public int DiasConStockPorLlegar { get; set; }
        public int MesesConStockPorLlegar { get; set; }
        public int? TotVtas2014Fre { get; set; }
        public int? TotVtas2015Fre { get; set; }
        public int? TotVtas2016Fre { get; set; }
        public int? TotVtas2017Fre { get; set; }
        public int? TotVtas2018Fre { get; set; }
        public int? TotVtas2019Fre { get; set; }
        public int? TotVtas2020Fre { get; set; }
        public int? TotVtas2021Fre { get; set; }
        public int? TotVtas2014Gci { get; set; }
        public int? TotVtas2015Gci { get; set; }
        public int? TotVtas2016Gci { get; set; }
        public int? TotVtas2017Gci { get; set; }
        public int? TotVtas2018Gci { get; set; }
        public int? TotVtas2019Gci { get; set; }
        public int? TotVtas2020Gci { get; set; }
        public int? TotVtas2021Gci { get; set; }
        public int CalificaVtas { get; set; }
        public int? TotVtasUlt12Mes { get; set; }
        public int? TotVtasUlt9Mes { get; set; }
        public int? TotVtasUlt6Mes { get; set; }
        public int? TotVtasUlt3Mes { get; set; }
        public int DiasSinStock { get; set; }
        public int CantSugeridaAComprar { get; set; }
        public decimal? PromCantVtasMes { get; set; }
        public int? StockFreddy { get; set; }
        public int? TotVtas2022Fre { get; set; }
        public int? TotVtas2022Gci { get; set; }
        public int? TotVtas2023Fre { get; set; }
        public int? TotVtas2023Gci { get; set; }
        public int? TotVtas2024Fre { get; set; }
        public int? TotVtas2024Gci { get; set; }
        public int? TotVtas2025Fre { get; set; }
        public int? TotVtas2025Gci { get; set; }
        public int? DiasDemoraImport { get; set; }
        public decimal? ValorUnitProm { get; set; }
        public int? CantAcomprarConfir { get; set; }
        public int? StockFre { get; set; }
        public int? DiasPromDemoraImpor { get; set; }
        public DateTime? UltFecIngreso1 { get; set; }
        public DateTime? UltFecPed1 { get; set; }
        public DateTime? FecUltPedPorLlegar { get; set; }
        public DateTime? FecUltllegaPorLlegar { get; set; }
        public DateTime? FechaSinStockPorLlegar { get; set; }
        public DateTime? FecProxPedidoLlega { get; set; }
        public DateTime? UltFecPed2 { get; set; }
        public DateTime? UltFecIngreso2 { get; set; }
        public string? SegRfm { get; set; }
        public decimal? PuntuaciónRfm { get; set; }
        public decimal? ScoreRfm { get; set; }
        public string? CodigoInterno { get; set; }
        public int? Idmarca { get; set; }
        public decimal? UltFobIngreso { get; set; }
        public decimal? UltCostoNacionalizado { get; set; }
        public int? StockGci { get; set; }
        public int? VtasPdv02 { get; set; }
        public int? VtasPdv04 { get; set; }
        public int? VtasPdv05 { get; set; }
        public int? VtasPdv06 { get; set; }
        public int? VtasPdv07 { get; set; }
        public int? VtasPdv09 { get; set; }
        public int? StockPdv02 { get; set; }
        public int? StockPdv04 { get; set; }
        public int? StockPdv05 { get; set; }
        public int? StockPdv06 { get; set; }
        public int? StockPdv07 { get; set; }
        public int? StockPdv09 { get; set; }
    }
}
