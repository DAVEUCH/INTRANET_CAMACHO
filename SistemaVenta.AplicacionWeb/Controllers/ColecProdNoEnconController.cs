...
{
    [Authorize]
    public class ColecProdNoEnconController : Controller
    {
        private readonly PostDbContext _context;

        public ColecProdNoEnconController(PostDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> Lista(int idAlmacen, DateTime fechaInicio, DateTime fechaFin)
        {
            ...
            var data = new List<VMColecProdNoEncon>();
            using (var conn = _context.Database.GetDbConnection())
            {
                await conn.OpenAsync();
                using (var command = conn.CreateCommand())
                {
                    command.CommandText = "usp_GC_M_Colectores_NO_Encontrados_show_Almacen_Fechas";
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        ...
                        {
                            data.Add(new VMColecProdNoEncon
                            {
